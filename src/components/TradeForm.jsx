...
import {
  ...
  createPrivateTrade,
  updateTrade,
} from "../firebaseRealtimeCrud"; // Import Firebase CRUD functions
...
const TradeForm = ({
  ...
  // Use ref to track saving state instead of global variable
  ...
  const determineSessionTag = (time) => {
    ...
    return "#OtherSession"; // Default for times outside these ranges
  };

  const handleSave = async () => {
    if (isSavingRef.current) return; // Prevent multiple saves
    isSavingRef.current = true; // Lock further saving attempts
    ...
    try {
      // Save all trades under users/userId/Trades
      if (existingTrade) {
        await updateTrade(userId, existingTrade.id, tradeData);
      } else {
        await createPrivateTrade(userId, tradeData); // Save under users/userId/Trades

        if (isPublic) {
          await createPublicTrade(userId, tradeData); // Also save in publicTrades if public
        }
      }
      onSave(tradeData); // Trigger save in parent component
      onClose(); // Close the form
    } catch (error) {
      console.error("Error saving trade: ", error);
    } finally {
      isSavingRef.current = false; // Unlock saving
    }
  };
      ...
      >
        ...
        >
          <InputField
            ...
            id="instrument"
            defaultValue={existingTrade?.instrument || ""}
          />
          <InputField
            icon="📊"
            placeholder="Strategy"
            id="strategy"
            defaultValue={existingTrade?.strategy || ""}
          />
          <InputField
            icon="📈"
            placeholder="Entry Point"
            id="entryPoint"
            defaultValue={existingTrade?.entryPoint || ""}
          />
          <InputField
            icon="📉"
            placeholder="Exit Point"
            id="exitPoint"
            defaultValue={existingTrade?.exitPoint || ""}
          />
          <InputField
            icon="💼"
            placeholder="Position Size"
            id="positionSize"
            defaultValue={existingTrade?.positionSize || ""}
          />
          <InputField
            icon="💰"
            placeholder="Profit/Loss"
            id="profitLoss"
            defaultValue={existingTrade?.profitLoss || ""}
          />
          <InputField
            icon="📅"
